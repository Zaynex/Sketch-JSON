...
function handleData(node, level, x, y, i) {
  ...
  if (
    ...
    if(classType == 'MSShapeGroup'&& ~name.indexOf('mb-line')) {
      ...
      // 有个问题 5px差值
      return Object.assign(tempFrame, {top: top - 5, bc: tempFill.bg, bs: 0, name: lineType})
    }
    if (classType === 'MSShapeGroup' && ~name.indexOf('Line')) {
      ...
      if (height == 3 && (height < width)) {
        return Object.assign({name: 'hr'}, tempFrame, { top: top - 3, height: bs, bc: bc, bs: 0})
      }
      return Object.assign({name: 've'}, { bc: bc, bs: 0, width: bs, left: left, height: height - 1, top: top})
    }
