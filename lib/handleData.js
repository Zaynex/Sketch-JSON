...
function handleData(node, level, x, y, i) {
  ...
  if (
    ...
    const resizingTabs = ['none', 'top', 'bottom', 'left', 'right', 'fix width', 'fix height']    
    let a = name.indexOf('fix-')
    if(a) {
      let tempResizingType = name.substr(a + 4).split('')
      tempResizingType = tempResizingType.map(v => resizingTabs[v])
      Object.assign(tempFrame, {resizing: tempResizingType})
    }
    /**
